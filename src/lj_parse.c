...
static void gola_resolve(LexState *ls, FuncScope *bl, MSize idx)
{
  ...
  for (; vg < vl; vg++)
	...
	lj_assertLS(strref(vg->name) != NAME_BREAK && strref(vg->name) != NAME_CONTINUE, "unexpected break");
  ...
  for (; v < ve; v++) {
	...
	if (bl->prev) {  /* Propagate goto, continue or break to outer scope. */
	  ...
	  if(bl->flags & FSCOPE_LOOP)
	    lj_assertLS(name != NAME_CONTINUE, "name != continue");
	  bl->prev->flags |=
...
static void fscope_loop_continue(FuncState *fs, BCPos pos)
{
  ...
  lj_assertFS(bl->flags & FSCOPE_LOOP, "continue not inside loop");
